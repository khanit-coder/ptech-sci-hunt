...
import { Radio, User, Clock, Award } from 'lucide-react';
...
export const RecentDiscoveries: React.FC<Props> = ({ discoveries, isLedMode = false }) => {
  const displayedDiscoveries = isLedMode ? discoveries.slice(0, 5) : discoveries;

  return (
    <div
      className={`w-full rounded-2xl sm:rounded-3xl border-2 border-passport-border bg-white shadow-passport-frame relative ${
        isLedMode ? 'p-2.5 sm:p-3.5' : 'p-4 sm:p-5'
      }`}
    >
      {/* Corner Rivet */}
      ...
      {/* Header Bar */}
      <div className="flex items-center justify-between pb-3 mb-3.5 border-b-2 border-slate-200">
      ...
      {/* Discovery Items Feed (Single Column, Multiple Rows) */}
      {discoveries.length === 0 ? (
        <div className="py-10 text-center text-slate-500 font-medium flex flex-col items-center gap-2">
          ...
          {displayedDiscoveries.map((disc, index) => (
            <div
              ...
              className={`p-3 sm:p-3.5 rounded-xl border-2 transition-all flex items-center justify-between gap-3 ${
                index === 0
                  ? 'bg-amber-50/80 border-mario-orange shadow-sm'
                  : 'bg-slate-50 border-slate-200 hover:border-slate-400 hover:bg-white'
              }`}
            >
              {/* Left: Icon, Code & Item Name */}
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0 shadow-xs border"
                  style={{
